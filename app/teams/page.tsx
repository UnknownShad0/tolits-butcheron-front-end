...
export default function TeamsPage() {
  ...
  useEffect(() => { apiFetch<Team[]>('/teams').then(setTeams).catch(console.error); }, []);

  return (
    <div>
      <h1 className="font-display text-4xl font-extrabold uppercase tracking-wide mb-6" style={{ color: cv.text }}>Teams</h1>
      {teams.length === 0 ? (
        <p style={{ color: cv.textDim }}>No teams yet. Add one in Admin.</p>
      ) : (
            ...
            <div key={team.id} className="rounded-xl p-5 flex gap-4 items-start border transition-all hover:opacity-90"
              ...
              <div>
                ...
                {team.players && team.players.length > 0 && (
                  <ul className="mt-2 text-sm list-disc list-inside" style={{ color: cv.textMuted }}>

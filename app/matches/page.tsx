...
export default function MatchesPage() {
  ...
  useEffect(() => { apiFetch<Match[]>('/matches').then(setMatches).catch(console.error); }, []);

  return (
    <div>
      <h1 className="font-display text-4xl font-extrabold uppercase tracking-wide mb-6" style={{ color: cv.text }}>Matches</h1>
      {matches.length === 0 ? (
        <p style={{ color: cv.textDim }}>No matches recorded yet.</p>
      ) : (
            ...
            <div key={m.id} className="rounded-xl p-4 flex items-center justify-between border"
              ...
              <div className="text-center flex-1 flex flex-col items-center gap-1">
                {m.home_team?.logo_path ? (
                  <Image src={`${STORAGE}/${m.home_team.logo_path}`} alt={m.home_team.name} width={36} height={36}
              ...
              <div className="text-center flex-1 flex flex-col items-center gap-1">
                {m.away_team?.logo_path ? (
                  <Image src={`${STORAGE}/${m.away_team.logo_path}`} alt={m.away_team.name} width={36} height={36}
                    className="rounded-full object-cover border-2" style={{ borderColor: cv.primary }} />
                ) : <span className="text-2xl">🏀</span>}
